...
class GatoRatonJuego(tk.Tk):
    ...
    def mostrar_pantalla_inicio(self):
        ...
        self.canvas.create_text(width // 2, height // 3, text="Gato y Ratón", font=("Arial", 36))
    ...
    def actualizar_juego(self):
        ...
        self.canvas.delete("gato", "raton")
        for x in range(0, width, cell_size):
            for y in range(0, height, cell_size):
                self.canvas.create_rectangle(x, y, x + cell_size, y + cell_size, outline="black")

        self.canvas.create_text(
            self.gato_pos[0] * cell_size + cell_size // 2, 
            self.gato_pos[1] * cell_size + cell_size // 2, 
            text="G", font=self.font, fill="black", tag="gato"
        )
        self.canvas.create_text(
            self.raton_pos[0] * cell_size + cell_size // 2, 
            self.raton_pos[1] * cell_size + cell_size // 2, 
            text="R", font=self.font, fill="black", tag="raton"
        )
